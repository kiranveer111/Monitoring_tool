    ...
    -- Table for Certificate Information
    -- Stores the latest certificate details for domain monitoring
    -- Uses a UNIQUE constraint on url_id for a one-to-one relationship
    CREATE TABLE IF NOT EXISTS `CertificateInfo` (
        `id` INT AUTO_INCREMENT PRIMARY KEY,
        `url_id` INT NOT NULL UNIQUE, -- One-to-one relationship with Urls table
        `expiry_date` DATETIME NULL, -- Changed to DATETIME to store full timestamp
        `days_remaining` INT NULL,
        `issuer` VARCHAR(512) NULL,
        `subject` VARCHAR(512) NULL,
        `status` ENUM('valid', 'warning', 'expired', 'unavailable', 'error', 'not_applicable', 'not_reachable') NOT NULL, -- Added 'unavailable', 'not_reachable'
        `error` TEXT NULL,
        `created_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        `updated_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
        FOREIGN KEY (`url_id`) REFERENCES `Urls`(`id`) ON DELETE CASCADE
